      ...
      `}</style>
            ...
            <nav className={`backdrop-blur-md border-b py-4 px-6 md:px-12 flex justify-between items-center z-40 relative ${darkMode ? 'bg-clinical-deep/95 border-white/5' : 'bg-white border-clinical-deep/10'}`}>
                <div className="flex items-center">
                    <img src="/HIPAA Landing Page/TWMMMinLogo_Trnsprnt_200 x 200px.png" alt="TWMM Logo" className={`h-12 w-auto mr-4 ${darkMode ? 'brightness-100' : 'brightness-0'}`} />
                    <div className="leading-none">
                        ...
                        <span className={`text-[10px] md:text-xs block tracking-[0.3em] uppercase mt-1 ${darkMode ? 'text-clinical-slate' : 'text-clinical-slate-dark'}`}>Management</span>
                    </div>
                </div>

                <div className="flex items-center space-x-4 md:space-x-10">
                    <div className={`hidden lg:flex space-x-8 text-[11px] font-bold uppercase tracking-widest ${darkMode ? 'text-clinical-slate' : 'text-clinical-slate-dark'}`}>
                        <a href="#training" className="hover:text-clinical-gold transition-colors">Annual Training</a>
                        <a href="#governance" className="hover:text-clinical-gold transition-colors">SRA & Governance</a>
                        <a href="#contact" className="bg-clinical-gold text-clinical-deep px-6 py-2.5 rounded-sm hover:bg-clinical-blue hover:text-white transition-all text-center">Enroll Now</a>
                    </div>
            ...
            <header className={`py-12 md:py-16 relative overflow-hidden ${darkMode ? 'hero-gradient text-white' : 'bg-clinical-blue text-white'}`}>
                <div className="container mx-auto px-6 relative z-10 grid lg:grid-cols-12 gap-12 items-center">
                    <div className="lg:col-span-7">
                        ...
                        <p className="text-lg md:text-xl text-clinical-slate mb-10 max-w-xl font-medium leading-relaxed">Standardized, online training for all clinicians and support staff. Notice of Privacy Practice (NPP) required to be implemented by February 16, 2026.</p>
                        ...
                        <div className={`border p-8 rounded-sm mb-10 inline-block shadow-2xl relative overflow-hidden backdrop-blur-sm ${darkMode ? 'bg-white/5 border-white/10' : 'bg-white/10 border-white/10'}`}>
                            <div className="absolute top-0 right-0 bg-green-600 text-white px-5 py-1.5 text-[10px] font-bold uppercase tracking-widest">Early Bird Pricing</div>
                            <div className="flex items-baseline space-x-4 mb-4">
                                <span className="text-clinical-gold text-6xl font-bold tracking-tighter">$599</span>
                                <span className="strikethrough text-clinical-slate text-2xl font-bold">$699</span>
                            </div>
                            <p className="text-sm font-bold uppercase tracking-widest text-clinical-gold mb-6 italic">Base HIPAA Compliance Package</p>
                            <ul className={`text-xs md:text-sm space-y-4 font-semibold uppercase tracking-wider ${darkMode ? 'text-clinical-slate' : 'text-white'}`}>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            <a href="#contact" className="bg-clinical-gold text-clinical-deep px-10 py-5 font-bold uppercase tracking-widest text-xs md:text-sm hover:bg-white transition-all text-center shadow-lg urgent-pulse">Sign Up & Start Immediately</a>
                    ...
                    <div className="grid md:grid-cols-4 gap-6">
                        {[
                            ...
                            <div key={idx} className={`p-10 border-t-2 border-clinical-gold shadow-sm ${darkMode ? 'bg-clinical-blue' : 'bg-clinical-light'}`}>
                                ...
                                <h5 className={`font-bold text-sm uppercase tracking-widest mb-4 ${darkMode ? 'text-white' : 'text-clinical-deep'}`}>{item.title}</h5>
                                <p className={`text-sm leading-relaxed font-medium ${darkMode ? 'text-clinical-slate' : 'text-clinical-slate-dark'}`}>{item.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="container mx-auto px-6">
                    <div className="grid lg:grid-cols-2 gap-20 items-center">
                        <div>
                            ...
                            <h3 className="text-4xl font-serif font-bold mb-8">SRA & BAA Optimization.</h3>
                            <p className="text-lg text-clinical-slate mb-12 leading-relaxed">Move beyond standard training into deep-dive clinical governance. Special pricing available for comprehensive risk analysis and vendor oversight.</p>
                            ...
                            <div className="grid sm:grid-cols-2 gap-8">
                                <div className="bg-white/5 p-8 border border-white/10 hover:bg-white/10 transition-colors text-center sm:text-left">
                                    ...
                                    <p className="text-sm text-clinical-slate">Liability-focused vendor contracts updated for 2026 regulations.</p>
                                </div>
                                <div className="bg-white/5 p-8 border border-white/10 hover:bg-white/10 transition-colors text-center sm:text-left">
                                    <h5 className="font-bold text-xs text-clinical-gold uppercase tracking-widest mb-4">Security Risk Analysis</h5>
                                    <p className="text-sm text-clinical-slate">Mandatory annual SRA with full mitigation roadmap for clinicians.</p>
                                </div>
                            </div>
                        </div>
            ...
            <section id="contact" className={`py-24 ${darkMode ? 'bg-clinical-deep' : 'bg-clinical-light'}`}>
                <div className="container mx-auto px-6 max-w-5xl">
                    <div className="text-center mb-16">
                        ...
                        <p className={`text-lg max-w-xl mx-auto italic font-bold tracking-tight ${darkMode ? 'text-clinical-slate' : 'text-clinical-slate-dark'}`}>Locked-in pricing: $599 Base Package + $25 per additional seat.</p>
                    </div>
                    ...
                    <div className={`shadow-2xl p-8 md:p-16 border-t-8 ${darkMode ? 'bg-clinical-blue border-clinical-gold' : 'bg-white border-clinical-deep'}`}>
                        <form onSubmit={handleSubmit} action="https://formspree.io/f/mbdjbkvv" method="POST" className="grid md:grid-cols-2 gap-10">
                            <div className="space-y-3">
                                <label className={`text-[11px] font-bold uppercase tracking-[0.2em] ${darkMode ? 'text-clinical-slate' : 'text-clinical-slate-dark'}`}>Practitioner/Admin Name</label>
                            ...
                            <div className="space-y-3">
                                <label className={`text-[11px] font-bold uppercase tracking-[0.2em] ${darkMode ? 'text-clinical-slate' : 'text-clinical-slate-dark'}`}>Clinical Practice Name</label>
                            ...
                            <div className="space-y-3 md:col-span-2">
                                <label className={`text-[11px] font-bold uppercase tracking-[0.2em] ${darkMode ? 'text-clinical-slate' : 'text-clinical-slate-dark'}`}>Professional Email</label>
                            ...
                            <div className="space-y-3 md:col-span-2">
                                <label className={`text-[11px] font-bold uppercase tracking-[0.2em] ${darkMode ? 'text-clinical-slate' : 'text-clinical-slate-dark'}`}>Enrollment Tier</label>
            ...
            <footer className="bg-clinical-deep py-20 text-white border-t border-white/5">
                <div className="container mx-auto px-6 grid md:grid-cols-3 gap-12 items-start">
                    <div className="text-center md:text-left">
                        <p className="text-xs uppercase tracking-[0.3em] text-clinical-slate mb-3">Transworld Medical Management, LLC.</p>
                        <p className="text-xs uppercase tracking-widest text-clinical-slate">Palm Harbor, Florida | Tampa Bay Region</p>
                    </div>
